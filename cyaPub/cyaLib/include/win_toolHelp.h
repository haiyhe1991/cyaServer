#ifndef __WIN_TOOL_HELP_H__
#define __WIN_TOOL_HELP_H__
...
public:
	// dwFlags，见WIN32 API CreateToolhelp32Snapshot()
	...
	BOOL ModuleFirst(PMODULEENTRY32 pme) const;/*模块儿*/
	...
	BOOL HeapListFirst(PHEAPLIST32 phl) const;/*堆链表*/
...
public:
	static BOOL EnableDebugPrivilege(BOOL fEnable = true);/*是否开放编译特权*/
	static BOOL ReadProcessMemory(DWORD th32ProcessID, LPCVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T cbRead, SIZE_T* lpNumberOfBytesRead);
};
...
inline CToolhelp::~CToolhelp()
{
	if (m_hSnapshot != INVALID_HANDLE_VALUE)
		VERIFY(CloseHandle(m_hSnapshot));
}

inline BOOL CToolhelp::IsValid() const
{
...
inline BOOL CToolhelp::ReadProcessMemory(DWORD th32ProcessID, LPCVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T cbRead, SIZE_T* lpNumberOfBytesRead)
{
	return Toolhelp32ReadProcessMemory(th32ProcessID, lpBaseAddress, lpBuffer, cbRead, lpNumberOfBytesRead);
}
...
inline BOOL CToolhelp::ModuleFind(PTSTR pszModName, PMODULEENTRY32 pme) const
{
	...
	for (BOOL fOk = ModuleFirst(pme); fOk; fOk = ModuleNext(pme))
	{
		if (0 == lstrcmpi(pme->szModule, pszModName)
			|| 0 == lstrcmpi(pme->szExePath, pszModName))
			return true;
	}
	return false;
}
